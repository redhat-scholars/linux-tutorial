...
[#downloadingrhel]
== Downloading RHEL
The first step to working with Red Hat Enterprise Linux is downloading the Universal Base Image from the https://developers.redhat.com/products/rhel/download[Red Hat Developer program].
On the Red Hat Developer page, scroll down to see the list of avaliable RHEL image downloads, and download the **x86_64 Boot iso**.
...
Select the default option to create a virtual hard disk now, and hit create. For the hard disk file type, select _VirtualBox Disk Image_ (VDI, reccomended for normal usage), and select _Dynamically allocated_ on the next prompt (also reccomended for daily usage).
...
The configuration items include selecting your time & date, connecting to a Red Hat account, software selection, specififying installation destination, and root password/user creation.
...
With the RHEL machine running, the last step is to log into your previously created account. We reccomend logging into `root` for this tutorial.
